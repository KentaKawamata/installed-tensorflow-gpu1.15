...
if [[ -z "${IMAGE_NAME}" ]]; then
    IMAGE_NAME=tensorflow_complete:latest
fi

# コンテナ名の設定
CONTAINER_NAME=$2
if [[ -z "${CONTAINER_NAME}" ]]; then
    CONTAINER_NAME=tensorflow_complete
fi
...
if [ -z "${DOPE_ID}" ]; then
    ...
    xhost +local:root
    docker run --gpus all -it --privileged --net host -v ${HOST_DIR}:${CONTAINER_DIR}:rw -v /tmp/.X11-unix:/tmp/.X11-unix:rw -v ~/.Xauthority:/root/.Xauthority:rw --env="DISPLAY=${DISPLAY}" --env="QT_X11_NO_MITSHM=1"  --env="XAUTHORITY=$XAUTH" --name=${CONTAINER_NAME} ${IMAGE_NAME} /bin/bash
